...
const resetPassword = async (req, res) => {
  try {
    ...
    let decoded;
    try {
      decoded = jwt.verify(token.trim(), process.env.JWT_SECRET_KEY);
    ...
    if (Date.now() >= decoded.exp * 1000) {
      console.log("Token expiration exceeded");
      return res.status(422).json({
        success: false,
    ...
    await userFound.save();  // The password will be automatically hashed by the pre-save middleware
